...
const auditDefinitionFiles=new Set(['scripts/copy-audit.mjs','scripts/human.mjs','public/human-ui.mjs']);
const patterns=[
...
const findings=[];
for(const file of files){const displayFile=relative(process.cwd(),file);if(auditDefinitionFiles.has(displayFile))continue;const text=await readFile(file,'utf8');for(const rule of patterns){for(const match of text.matchAll(rule.re)){const line=text.slice(0,match.index).split('\n').length;findings.push({file:displayFile,line,rule:rule.label,match:match[0]});}}}
